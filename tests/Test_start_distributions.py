...
class TestStartDistributions(InDirTest):
    ...
    def test_start_age_distribuions_from_empty_spinup(self):
        ...
        t0_index = int(n_steps/2)
        t0 = times[t0_index]
        a_dens_func_t0, pool_contents = start_age_distributions_from_empty_spinup(  # noqa: E501
            srm,
            t_max=t0,
            parameter_dict=parameter_dict,
            func_set=func_set
        ...
        # for this particular example we are only interrested in ages that
        # are smaller than t_max
        # the particular choice ages=times means that t_0_ind is the same
        # in both arrays
        ages = times
        ...
        for n in range(srm.nr_pools):
            fig = smr.plot_3d_density_plotly(
                "pool {0}".format(n), pool_dens_data[:, :, n], ages
            )
...
#            smr.add_equilibrium_surface_plotly(fig)
            ...
            # make sure that the values for the model run at t0 conince with
            # the values computed by the function returned by the function
            # under test
            ...
            # make sure that the density is zero for all values of age
            # bigger than t0
    ...
    def test_numeric_steady_state(self):
        ...
        res = compute_fixedpoint_numerically(
            srm,
            t0=0,
            x0=np.array([1, 1]),
            parameter_dict={},
        ...
        F_sym = srm.F
        F_func = numerical_function_from_expression(
            F_sym,
            tup=(C_0, C_1, t),
            parameter_dict={},
            func_set=func_set
    ...
    def test_compute_start_age_moments_from_steady_state(self):
        ...
        self.assertEqual(age_moment_vector.shape, (2, 2))
        # we only check the expectation values since B is the identity the
        # number are the same as in the input fluxes
        ref_ex = np.array([1, 2])
        for pool in range(srm.nr_pools):
            self.assertTrue(np.allclose(age_moment_vector[:, pool], ref_ex))
...
#    # to let the buildbot fail we set the exit value !=0 if either a failure
#    # or error occurs
