...
class TestDiscreteModelRun(InDirTest):
    def test_from_SmoothModelRun(self):
        ...
        #tup =  (srm.time_symbol,) + tuple(state_vector) 
        #num_rhs_many_args=numerical_function_from_expression(rhs,tup,pardict,{})
        #def f(t,X):
        #    return num_rhs_many_args(t,*X).reshape(2,)

        #sf=solve_ivp(fun=f,t_span=[0,t_max],y0=start_values,max_step=delta_t,vectorized=False,method='LSODA')
