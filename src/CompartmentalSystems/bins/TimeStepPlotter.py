...
def plot_external_gains(external_input_numbers, tss, nTs, nTp, max_z):
    for key, number in external_input_numbers.items():
        ...
        with SinglePlotFigureHandler("external_gains_pool" + tuplestr + ".pdf") as ax:
            ax.set_title(
                Template("external gains for pool  ${key}").substitute(key=key)
            )
...
def plot_figure_per_pipe(rectangles, max_Ts, max_Tp, max_z, title_stem, facecolors):
    ...
    for key, f in rectangles.items():
        tupelstr = "{first_arg:01d}_{second_arg:01d}".format(
            first_arg=key[0], second_arg=key[1]
        )
        with SinglePlotFigureHandler(
            title_stem.replace(" ", "_") + tupelstr + ".pdf"
        ) as ax:
...
def plot_figure_per_pool(rectangles, max_Ts, max_Tp, max_z, title_stem, facecolors):
    ...
    for key, f in rectangles.items():
        with SinglePlotFigureHandler(
            title_stem.replace(" ", "_")
            + "{first_arg:01d}".format(first_arg=key)
            + ".pdf"
        ) as ax:
            ax.set_title(title_stem + str(key))
            f.plot_bins(ax, facecolors=facecolors)
...
def plot_contents_and_gains(
    contents, internal_gains, external_gains, max_Ts, max_Tp, max_z
):
    for content_pool_key, content in contents.items():
        with SinglePlotFigureHandler(
            "contents_and_gains_pool_"
            + "{first_arg:01d}".format(first_arg=content_pool_key)
            + ".pdf"
        ) as ax:
            ax.set_title(
                Template("contents of pool ${key} and all gains").substitute(
                    key=content_pool_key
                )
            )
            ...
            for receiver, gain in internal_gains.items():
                if receiver == content_pool_key:
                    gain.plot_bins(
                        ax, 0, facecolors=gain_facecolors, offset_field=offset
                    )
                    offset += gain.arr
...
def plot_contents_and_losses(
    remainders, internal_losses, external_losses, max_Ts, max_Tp, max_z
):
    for content_pool_key, this_pool_remainder in remainders.items():

        with SinglePlotFigureHandler(
            "contents_and_losses_pool_"
            + "{first_arg:01d}".format(first_arg=content_pool_key)
            + ".pdf"
        ) as ax:
            ax.set_title(
                Template("contents of pool ${key} and all losses").substitute(
                    key=content_pool_key
                )
            )
...
def plot_internal_gains(gains, max_Ts, max_Tp, max_z):
    ...
    for key, f in gains.items():
        ...
        with SinglePlotFigureHandler("internal_gains_pool" + tuplestr + ".pdf") as ax:
            ax.set_title(
                Template("internal gains for pool  ${key}").substitute(key=key)
            )
...
class TimeStepPlotter:
    ...
    def plot_pdfs(self):
        ...
        plot_figure_per_pool(
            res, max_Ts, max_Tp, max_z, "initial content pool ", content_facecolors
        )
        ...
        plot_contents_and_losses(
            remainder, internal_losses, external_losses, max_Ts, max_Tp, max_z
        )
        plot_figure_per_pool(
            external_losses,
            max_Ts,
        ...
        plot_figure_per_pool(
            ...
            max_Tp,
            max_z,
            "content after removel of internal and external losses pool ",
            content_facecolors,
        )
        res.shift()
        plot_figure_per_pool(
            res, max_Ts, max_Tp, max_z, "contents after shift", content_facecolors
        )

        plot_contents_and_gains(
            res, gains, external_input_numbers, max_Ts, max_Tp, max_z
        )
        res.receive(gains)
        res.receive_external(external_input_numbers)
        plot_figure_per_pool(
            res, max_Ts, max_Tp, max_z, "updated contents pool ", content_facecolors
        )
        #
