...
class CSPlotter:
    ...
    def plot_pools_and_fluxes(self, ax, black_and_white=False):
        ...
        for i in range(nr_pools):
            ...
            # plot influx
            if i in inputs.keys():
                pool.plot_input_flux(ax, self.pipe_colors[inputs[i]], black_and_white)

            # plot outflux
            if i in outputs.keys():
                patch = pool.plot_output_flux(
                    ax, self.pipe_colors[outputs[i]], black_and_white
                )
                #                patch.set_label(f"{i}->{-1}")
                patch._name = f"{i}->{-1}"
        ...
        for key in internal_fluxes.keys():
            i, j = key
            patch = pools[i].plot_internal_flux_to(
                ax, pools[j], self.pipe_colors[internal_fluxes[key]], black_and_white
            )
            #            patch.set_label(f"{i}->{j}")
            patch._name = f"{i}->{j}"
