...
class DiscreteModelRun:
    ...
    def from_fluxes(cls, start_values, times, net_Us, net_Fs, net_Rs):
        Bs = cls.reconstruct_Bs_without_xs(start_values, net_Us, net_Fs, net_Rs)
        return cls.from_Bs_and_net_Us(start_values, times, Bs, net_Us)
    ...
    def from_fluxes_2(cls, start_values, times, Us, Fs, Rs):
        ...
        Bs = cls.reconstruct_Bs_without_xs_2(start_values, Us, Fs, Rs)
        return cls.from_Bs_and_Us_2(start_values, times, Bs, Us)
    ...
    def from_iterator(cls, tsit):
        ...
        n_pools = len(x_0)
        net_Bs, net_Us, times = zip(
            *((ts.B + np.eye(n_pools), ts.u.reshape(-1), ts.t) for ts in tsit)
        )
        ...
        mr = cls.from_Bs_and_net_Us(x_0, times_arr, net_Bs_arr[:-1], net_Us_arr[:-1])
        # from IPython import embed;embed()
        return mr

    @classmethod
    def from_euler_forward_smooth_reservoir_model(
        cls, srm, par_dict, func_dict, delta_t, number_of_steps, start_values
    ):
        ...
        B = xi * T * N
        sym_B = hr.euler_forward_B_sym(B, t, delta_t, it) * delta_t
        sym_B_dt = sym_B
        sym_u_dt = hr.discrete_time_sym(u, t, delta_t, it) * delta_t
        num_B = hr.numerical_array_func(x, it, sym_B_dt, par_dict, func_dict)
        num_u = hr.numerical_1d_vector_func(x, it, sym_u_dt, par_dict, func_dict)
    ...
    @classmethod
    def from_B_and_u_funcs(cls, x_0, B_func, u_func, number_of_steps, delta_t):
        i_min = 0
    ...
    def restrict_to_pools(self, pool_nrs: np.ndarray) -> "__class__":
        ...
        dmr_restricted = self.__class__.from_Bs_and_net_Us(
            start_values_restricted, self.times, Bs_restricted, net_Us_restricted
        )
    ...
    def net_Us(self):
        n = len(self.Bs)
        return np.array(
            [self.xs[k + 1] - np.matmul(self.Bs[k], self.xs[k]) for k in range(n)]
        )
    ...
    def time_bin_index(self, t: float) -> int:
        """
    ...
    def from_SmoothModelRun(cls, smr, nr_bin):
        ...
        # not equidistant
        data_times = np.linspace(smr.times[0], smr.times[-1], nr_bin + 1)
        return cls(
            data_times,
    ...
    @classmethod
    def reconstruct_Bs_without_xs(cls, start_values, Us, Fs, Rs, verbose=False):
        x = start_values
        Bs = np.nan * np.ones_like(Fs)
        go_through = tqdm(range(len(Rs))) if verbose else range(len(Rs))
        for k in go_through:
            try:
    ...
    def fake_xss(self, nr_time_steps):
        ...
        if np.all(self.net_Us == 0):
            raise (
                DMRError("Cannot fake xss, because there are no inputs to the systems")
            )
    ...
    def fake_eq_14C(self, nr_time_steps, F_atm, decay_rate, lim, alpha=None):
        ...
        for pool in range(self.nr_pools):
            ...
            def p0_pool_14C_quad(a):
                res = (
                    (F_atm(a) / 1000.0 + 1)
                    * p0_pool(int(a / self.dt))
                    / self.dt  # make masses to density
            ...
            res_quad = solve_ivp(
                lambda a, y: p0_pool_14C_quad(a), (0, lim), np.array([0])
            )
    ...
    def fake_start_age_moments(self, nr_time_steps, up_to_order):
        ...
        for n in range(1, up_to_order + 1):
            ...
            start_m_moment = np.zeros(nr_pools)
            for k in range(n + 1):
                start_m_moment += stirling(n, k) * self.fake_start_m_factorial_moment(
                    k, nr_time_steps
                )
    ...
    def system_age_moment(self, order, start_age_moments, mask=False):
        if not isinstance(mask, bool):
            mask_over_time = np.repeat(mask.reshape(1, -1), len(self.times), axis=0)
        else:
            mask_over_time = mask
    ...
    def backward_transit_time_moment(
        self, order: int, start_age_moments: np.ndarray, mask: np.ndarray = False
    ) -> np.ndarray:
        ...
        """
        if not isinstance(mask, bool):
            mask_over_time = np.repeat(mask.reshape(1, -1), len(self.times) - 1, axis=0)
        else:
            mask_over_time = mask
        ...
        age_moment_vector = self.age_moment_vector(order, start_age_moments)[:-1, :]
        pool_axis = 1
        return np.array(
    ...
    def age_densities_1_single_value_func(
        self, start_age_densities_of_bin: Callable[[int], np.ndarray]
    ...
    def _age_densities_1_func(self, start_age_densities_of_bin_index):
        ...
        def p1(age_bin_indices):
            ...
            if len(age_bin_indices) > 0:
                ...
                for it in range(nt):
                    ...
                    sais = np.stack([p0(ai - it) for ai in age_bin_indices], axis=1)
                    # print(sais.shape)
    ...
    def _age_densities_2_func(self):
        ...
        def p2(age_bin_indices):
            ...
            if len(age_bin_indices) > 0:
                vals = np.stack(
                    [
                        np.stack([p2_sv(ia, it) for it in range(nt)], axis=0)
                        for ia in age_bin_indices
                    ]
                )
    ...
    #    return prev_age
    def age_quantile_at_time(self, q, t, pools, start_age_densities_of_bin) -> float:
    ...
    def _H_sv(self):
        ...
        def h(ai, ti):
            ...
            m = np.matmul(Phi(ti, ti - (ai + 1)), x_ti_minus_ai_plus_1).reshape(
                (self.nr_pools,)
            )
    ...
    def backward_transit_time_quantiles(self, q, P0, times=None, mask=False):
        ...
        if not isinstance(mask, bool):
            mask_over_time = np.repeat(mask.reshape(1, -1), len(times) - 1, axis=0)
        else:
            mask_over_time = mask
    ...
    def CS_through_time(self, n0, mask=False, verbose=False):
        x = len(self.times)
        go_through = trange(x) if verbose else range(x)

        return np.array([self.CS(n0, N, mask) for N in go_through])
