...
setwd("/home/hmetzler/GitHub/CompartmentalSystems/notebooks/nonl_gcm_3p_many_params")
getwd()
...
for (version in 0:9){
  print(version)
  version <- 1
  folder <- paste0("output_data_v", version)
  folder <- paste0(folder, "/")
  file <- paste0(folder, "age_mean.csv")

  age_mean_data <- read.csv(file)
  age_mean <- age_mean_data[age_mean_data$pool==pool, c('time', 'value')]
  plot(age_mean)
}


load_mean_age_data <- function(version){
