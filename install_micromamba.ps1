...
# but is a known problem at the time of writing and does affect us.
 micromamba install -y -c conda-forge matplotlib mpmath numpy plotly scipy sympy tqdm frozendict networkx pandas python-igraph pycairo pip
...
# you need some extra packages which can be installed by uncommenting the following line:
 micromamba install -y -c conda-forge dask[distributed] jupyterlab jupytext nbformat nbmake notebook pytest zarr
...
#  micromamba install -y -c conda-forge sphinx sphinx-autodoc-typehints
...
# if you have checked out these repos and installed the code in developer mode
 pip install --no-deps git+https://github.com/MPIBGC-TEE/testinfrastructure.git#egg=testinfrastructure git+https://github.com/MPIBGC-TEE/LAPM.git#egg=LAPM

# The following line installs the package without checking out the repository directly from github
 pip install --no-deps git+https://github.com/MPIBGC-TEE/CompartmentalSystems.git@test#egg=CompartmentalSystems
